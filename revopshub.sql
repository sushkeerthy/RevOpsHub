...
tickets AS (
    ...
    WHERE t.product_name LIKE '%Elite Edge%'
      AND t.status IN ('Scheduled', 'Attended', 'No Show', 'assigned', 'scheduled', 'reserved')
),
    ...
    FROM (
        ...
        WHERE at.attendee_id IN (
            ...
            WHERE product_name LIKE '%Elite Edge%'
              AND status IN ('Scheduled', 'Attended', 'No Show', 'assigned', 'scheduled', 'reserved')
        )
    ) x
...
dim_customer AS (
    ...
    -- ============================================================
    -- Previous logic: DWH only — re-enable if HubSpot causes issues
    -- SELECT
...
dim_employee AS (
    SELECT
        de.EmployeeName
            COLLATE Latin1_General_100_BIN2_UTF8  AS EmployeeName,
        de.Email                                  AS Email
...
netsuite_names AS (
    SELECT
        ...
        CASE ns.SalesRepName
            WHEN 'Samuel Van Dong'    THEN 'Sam Van Dong'
            WHEN 'Sam V Dong'         THEN 'Sam Van Dong'
            WHEN 'Ricardo Nieves'     THEN 'Ricardo Nieves Salas'
            WHEN 'Marcellous Hazzard' THEN 'Marcell Hazzard'
            WHEN 'Matthew  Pegios'    THEN 'Matthew Pegios'
            WHEN 'Joshua Montanez'    THEN 'Josh Montanez'
            WHEN 'John Carrillo'      THEN 'JC Carrillo'
            WHEN 'Joshua Cervantes'   THEN 'Josh Cervantes'
            WHEN 'Josh Turney'        THEN 'Joshua Turney'
